...
function renderSharedPost(sharedPost, sharedProfileMap) {
  ...
  return `
    <div style="margin-top:12px;padding:14px;background:var(--bg-input);border:1px solid var(--border);border-radius:12px;cursor:pointer;" onclick="window.location.href='/comments.html?post=${sharedPost.id}'">
...
async function handleReport(postId) {
  ...
  try {
    const { data: existing } = await window.db
      .from('reports').select('id').eq('reporter_id', currentUser.id).eq('target_id', postId).eq('target_type', 'post').single();
    if (existing) {
...
function renderPost(post, profileMap, communityMap, reactionMap, commentCountMap, myReactionMap, viewCountMap, pollMap, shareCountMap, sharedPostMap, sharedProfileMap) {
  ...
  const topEmojis = REACTIONS.filter(r => postReactions[r.type] > 0).sort((a, b) => (postReactions[b.type] || 0) - (postReactions[a.type] || 0)).slice(0, 3).map(r => r.emoji).join('');
  ...
  const sharedPostHtml = post.shared_post_id && sharedPostMap ? renderSharedPost(sharedPostMap[post.shared_post_id] || null, sharedProfileMap || {}) : '';
  const postContent = post.shared_post_id ? (post.content || '').replace(/\n\n🔁 \[Reposted\]$/, '').trim() : (post.content || '');
...
function attachPostListeners() {
  ...
  document.querySelectorAll('.reaction-option').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      document.querySelector(`.reaction-picker[data-post-id="${btn.dataset.postId}"]`).style.display = 'none';
      handleReaction(btn.dataset.postId, btn.dataset.type);
    });
  });
  ...
  document.querySelectorAll('.edit-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      document.querySelector(`.post-menu-dropdown[data-post-id="${btn.dataset.postId}"]`).style.display = 'none';
      showEditForm(btn.dataset.postId, btn.dataset.content);
    });
  });
...
async function handleCreatePost() {
  ...
  try {
    ...
    await fireMentionNotifications(content, post.id, currentUser.id);
